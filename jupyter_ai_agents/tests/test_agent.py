...
from jupyter_ai_agents.agents.langchain.base.agent_base import NbModelBaseAgent


async def test_default_content(ws_server):
    room = uuid.uuid4().hex
    async with NbModelBaseAgent(f"{ws_server}/{room}") as agent:
...
async def test_set_user_prompt(ws_server):
    ...
    async with NbModelClient(room_url) as client:
        async with NbModelBaseAgent(room_url) as agent:
